...
@section('title', 'Modifier retour')

@section('content')
<div class="max-w-3xl mx-auto bg-white/90 rounded-2xl shadow-lg p-6">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Modifier le retour</h1>
    ...
    <form action="{{ route('retours.update', $retour->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select id="article" name="stock_id" required class="w-full border border-gray-200 rounded-lg px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300 transition">
                ...
                @foreach($stocks as $stock)
                    <option value="{{ $stock->id }}" {{ $retour->stock_id == $stock->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="text" id="type" name="type" value="{{ $retour->type }}" readonly class="w-full border border-gray-200 rounded-lg px-3 py-2 text-gray-400 shadow-sm">
        </div>
        ...
        <div>
            ...
            <input type="number" id="quantite" name="quantite" value="{{ old('quantite', $retour->quantite) }}" required min="1" class="w-full border-gray-300 rounded-lg px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300 transition">
            @error('quantite')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="datetime-local" id="date_movement" name="date_movement" 
                value="{{ old('date_movement', $retour->date_movement->format('Y-m-d\TH:i')) }}" 
                required 
        ...
        <div>
            ...
            <input type="text" id="note" name="note" value="{{ old('note', $retour->note) }}" placeholder="Entrer une remarque" class="w-full border border-gray-200 rounded-lg px-3 py-2 shadow-sm">
            @error('note')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex justify-end gap-4">
            <a href="{{ route('retours.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold px-6 py-2 rounded-xl shadow transition">
                Annuler
            </a>
