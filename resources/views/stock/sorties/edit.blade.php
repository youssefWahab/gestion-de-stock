...
<div class="max-w-3xl mx-auto bg-white/90 rounded-2xl shadow-lg p-6">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Modifier sortie</h1>
    ...
    <form action="{{ route('entrees.update', $sortie->id) }}" method="POST" class="space-y-6">
        {{--  --}}
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select id="article" name="stock_id" required class="w-full border border-gray-200 rounded-lg px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300 transition">
                ...
                @foreach($stocks as $stock)
                    <option value="{{ $stock->id }}" {{ $sortie->stock_id == $stock->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="text" id="type" name="type" value="{{ $sortie->type }}" readonly class="w-full border border-gray-200 rounded-lg px-3 py-2 text-gray-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300 transition">
        </div>
        ...
        <div>
            ...
            <input type="number" id="quantite" name="quantite" value="{{ old('quantite', $sortie->quantite) }}" required min="1" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-gray-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300 transition">
            @error('quantite')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" id="reference" name="reference" value="{{ old('reference', $sortie->reference) }}" placeholder="Entrer une référence" class="w-full border border-gray-200 rounded-lg px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300 transition">
            @error('reference')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="datetime-local" id="date_entree" name="date_movement" value="{{ old('date_movement', $sortie->date_movement->format('Y-m-d\TH:i') ) }}" required class="w-full border-gray-300 rounded-lg px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300 transition">
            @error('date_movement')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" id="note" name="note" value="{{ old('note', $sortie->note) }}" placeholder="Entrer une remarque" class="w-full border border-gray-200 rounded-lg px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300 transition">
            @error('note')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
