...
@section('content')
    <div class="bg-white/80 rounded-2xl shadow-2xl py-8 px-5 border border-blue-100 backdrop-blur-lg">
        ...
        <h1 class="text-4xl font-black text-center mb-10 text-gradient bg-clip-text text-transparent bg-gradient-to-r from-indigo-700 via-blue-600 to-purple-500">
            Gestion des Articles pour la demande  N° {{ $demande->numBonCommande }}
        </h1>
        ...
        {{-- formulaire d'ajouter nouvelle article --}}
        <form action="{{ route('article-achat.store') }}" method="POST" class="bg-white p-4 rounded-2xl shadow-md flex flex-col md:flex-row gap-3 mb-6 items-end">
            @csrf
            <input type="hidden" name="demande_achat_id" value="{{ $demande->id }}">
            <div class="flex-1 flex flex-col w-full md:w-full">
                ...
                <input type="text" name="articleDemande" id="articleDemande" placeholder="Nom de l'article"
            ...
            <div class="flex flex-col w-full md:w-24">
                ...
                <input type="text" name="unite" id="unite" placeholder="Unité" class="w-full px-4 py-2 border border-purple-200 rounded-xl focus:ring-2 focus:ring-purple-400 focus:outline-none transition"/>
            </div>
            <div class="flex flex-col w-full md:w-28">
                <label for="prix" class="mb-1 text-gray-600 font-medium">Prix</label>
                <input type="number" name="prix" id="prix" placeholder="0" step="0.01"
                    class="w-full px-4 py-2 border border-purple-200 rounded-xl focus:ring-2 focus:ring-purple-400 focus:outline-none transition"/>
            </div>
                    ...
                    @forelse($demande->articles as $article)
                        <tr>
                            ...
                            <td class="px-4 py-2 text-center flex justify-center gap-2">
                                <a href="{{ route('article-achat.edit', $article->id) }}" class="px-3 py-1.5 rounded-lg text-xs bg-green-100 text-green-800 hover:text-green-900 font-semibold">
                                    <i class="fa-solid fa-pen"></i>
                                </a>
                                <form action="{{ route('article-achat.destroy', $article->id) }}" method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer cet article ?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
        ...
        <div class="mt-6 flex justify-end">
            <a href="{{ route('demande-achat.edit',$demande->id) }}" class="inline-flex px-4 py-2 rounded-xl bg-gray-200 text-gray-800 font-semibold hover:bg-gray-300 transition items-center gap-2">
