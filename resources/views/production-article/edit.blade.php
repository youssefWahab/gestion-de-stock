...
@section('content')
    <div class="bg-white/80 rounded-2xl shadow-2xl p-10 border border-blue-100 backdrop-blur-lg max-w-2xl mx-auto">
        ...
        <form action="{{ route('production-article.update', $article->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                <label for="unite" class="block text-sm font-semibold text-gray-600 mb-1">Unité</label>
            ...
            <div>
                <label for="prix" class="block text-sm font-semibold text-gray-600 mb-1">Prix</label>
                <input type="text" id="prix" name="prix" value="{{ old('prix', $article->prix) }}" class="w-full border border-purple-200 rounded-xl px-4 py-2 focus:ring-2 focus:ring-purple-400 focus:outline-none">
            </div>

            <div class="flex justify-end gap-4 mt-6">
                <a href="{{ route('production-article.index') }}"
                class="px-6 py-2 rounded-xl bg-gray-200 text-gray-800 font-semibold hover:bg-gray-300 transition flex items-center gap-2">
                <i class="fa-solid fa-arrow-left"></i> Retour aux articles des production
                </a>
